...
1. Feature Plane (`FeatureEngine`) sits after `LOBEngine` in runtime and replay/backtest paths.
2. `HftBacktestAdapter` feature-first mode allows backtests to use the same shared feature kernels as live.
3. Shared microstructure features computed in governed feature kernels, not in individual strategies.
4. Strategy-level decision logic remains in strategy modules (feature plane computes shared inputs only).

Implemented:

- `FeatureEngine` runtime component: 16 features (8 stateless + 8 rolling), `HFT_FEATURE_ENGINE_ENABLED=1`
- Strategy pull APIs: `StrategyContext.get_feature_tuple(symbol)` for O(1) hot-path access
- `HftBacktestAdapter` feature-first mode (`lob_feature`) with `feature_tuple_source` wiring
- Feature ABI/versioning (`FEATURE_SET_VERSION = "lob_shared_v1"`) and parity gates in Gate D

Remaining production backlog:

- Rust transport for feature kernels (zero-copy boundary)
- Production kernel promotion workflow (Python reference -> Rust kernel migration)
...
- canonical DDL path: `src/hft_platform/migrations/clickhouse/`
...
7. Shared promoted microstructure features must preserve parity across research replay, `hftbacktest`, and live runtime for the same feature set/version (enforced via `FEATURE_SET_VERSION` and Gate D FSV check).
...
2. Audit bootstrap gap (RESOLVED)
...
5. Research/live feature drift risk for shared microstructure factors (MITIGATED)

- FeatureEngine (Phase 18) provides shared feature kernels for research and runtime.
- Feature set versioning (`FEATURE_SET_VERSION`) and Gate D FSV check enforce parity.
- Remaining risk: Rust kernel migration not yet done; Python-only kernels are the current baseline.
...
| CE2-07 | Add gateway metrics/alerts/dashboard and SLO definitions            | ✅ Implemented |
| CE2-08 | Multi-runner integration test and chaos test for gateway outages    | ✅ Implemented |
| CE2-09 | Add active/standby gateway failover and leader lease control        | ✅ Implemented |
| CE2-11 | Enforce quote schema lock (`quote_version=v1`) with guardrails      | ✅ Implemented |
...
| CE3-03 | Scale out WAL loader workers and shard assignment policy        | ✅ Implemented |
| CE3-04 | Define replay safety contract (ordering + dedup + manifest)     | ✅ Implemented |
| CE3-06 | Add WAL SLO metrics, alerts, and dashboards                     | ✅ Implemented |
| CE3-07 | Outage drills: ClickHouse down, slow, and WAL growth recovery   | ✅ Implemented |
