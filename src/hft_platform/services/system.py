...
class HFTSystem:
    ...
    async def run(self):
        ...
        try:
            ...
            # Disable GC during active trading to avoid stop-the-world pauses.
            if os.getenv("HFT_GC_DISABLE_TRADING", "1").lower() not in {"0", "false", "no", "off"}:
                gc.disable()
                gc.collect()  # one final collection before disabling
