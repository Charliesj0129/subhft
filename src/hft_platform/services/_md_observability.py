...
class _SupportsObservability(Protocol):
    feature_engine: FeatureEngine | None
    metrics_registry: MetricsRegistry | None
    _trace_sampler: Any
    _feature_latency_counter: int
    _feature_metrics_counter: int
    _feature_latency_sample_every: int
    _feature_metrics_sample_every: int
    _feature_latency_metric_child: Any | None
    _feature_update_metric_children: dict[tuple[str, str], Any]
    _feature_quality_flag_metric_children: dict[str, Any]
    _feature_set_id_cached: str
    _feature_shadow_engine: FeatureEngine | None
    _feature_shadow_counter: int
    _feature_shadow_sample_every: int
    _feature_shadow_abs_tolerance: float
    _feature_shadow_mismatch_counter: int
    _feature_shadow_warn_every: int
    _feature_shadow_checks_metric_children: dict[tuple[str, str], Any]
    _feature_shadow_mismatch_metric_children: dict[tuple[str, str], Any]
...
class MarketDataObservabilityMixin:
    ...
    def _emit_trace(self: Any, stage: str, trace_id: str, payload: dict[str, Any]) -> None:
    ...
    def _record_shioaji_crash_signature(self: Any, text: str | None, *, context: str) -> None:
    ...
    def _record_feature_metrics(  # noqa: C901
        ...
        if self._feature_latency_counter % self._feature_latency_sample_every == 0:
            try:
                if self._feature_latency_metric_child is None and hasattr(metrics_registry, "feature_plane_latency_ns"):
                    self._feature_latency_metric_child = metrics_registry.feature_plane_latency_ns
    ...
    def _emit_feature_shadow_check_metric(self: Any, result: str) -> None:
        metrics_registry: MetricsRegistry | None = getattr(self, "metrics_registry", None)
