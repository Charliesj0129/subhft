...
def configure_risk() -> Dict[str, Any]:
    ...
    return {"max_position_lots": max_pos, "daily_loss_limit": max_loss, "kill_switch_enabled": kill_switch}
...
''')
        ...
        strat_config = {"id": fname, "module": f"hft_platform.strategies.{fname}", "class_name": cname, "enabled": True}
...
def run_wizard():
    ...
    print(
        f"Universe: {len(config_state['symbols'])} symbols ({', '.join(s['code'] for s in config_state['symbols'][:5])}...)"
    )
    print(
        f"Risk: Max {config_state['risk']['max_position_lots']} lots, Loss Limit ${config_state['risk']['daily_loss_limit']}"
    )
