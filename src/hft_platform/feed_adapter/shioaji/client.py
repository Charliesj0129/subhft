"""ShioajiClient — core low-level Shioaji broker adapter.

Canonical location: ``hft_platform.feed_adapter.shioaji.client``
...
def _sdk() -> Any:
    ...
    Tests patch ``hft_platform.feed_adapter.shioaji_client.sj`` so we must
    read the attribute at call time rather than capturing it at import time.
    This indirection keeps the shim-based test patches working after the
    class was relocated to this module.
    """
    shim = sys.modules.get("hft_platform.feed_adapter.shioaji_client")
    if shim is not None:
