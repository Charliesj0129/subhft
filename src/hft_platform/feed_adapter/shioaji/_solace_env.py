"""Observability for the Shioaji SDK's Solace (pysolace) reconnect parameters.
...
The platform leaves these at SDK defaults and never logged them, so the
...
def read_solace_reconnect_params() -> dict[str, Any] | None:
    ...
    Returns ``None`` if ``shioaji.config`` cannot be imported (e.g. the SDK is
    not installed in this environment). Missing attributes map to ``None``.
    """
    try:
