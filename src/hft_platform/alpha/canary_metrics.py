"""Canary metrics source abstraction for CanaryMonitor.

Provides pluggable data sources (ClickHouse, Redis, Hybrid) that supply
live metrics required by CanaryMonitor.evaluate().
"""
...
class CanaryMetricsSource(Protocol):
    """Protocol for live canary metrics retrieval."""

    def get_live_metrics(self, alpha_id: str) -> dict[str, Any]:
        """Return live metrics dict for the given alpha.

        Expected keys (subset may be present):
          - slippage_bps: float
          - drawdown_contribution: float
          - execution_error_rate: float
          - sessions_live: int
          - sharpe_live: float (optional)
        """
        ...
