"""Canonical alpha discovery and in-memory registry boundary."""
...
class AlphaDiscoveryRegistry:
    """Registry core whose implementation depends only on canonical alpha contracts."""
    ...
    def list_by_status(self, status: AlphaStatus) -> list[AlphaManifest]:
        return [alpha.manifest for alpha in self._alphas.values() if alpha.manifest.status == status]

    def list_by_tier(self, tier: AlphaTier) -> list[AlphaManifest]:
        return [alpha.manifest for alpha in self._alphas.values() if alpha.manifest.tier == tier]

    def get(self, alpha_id: str) -> AlphaProtocol | None:
        return self._alphas.get(alpha_id)

    def manifests(self) -> list[AlphaManifest]:
        return [alpha.manifest for alpha in self._alphas.values()]
