...
from hft_platform.features.liquidity import RollingKyleLambda, ShadowQueueEstimator
from hft_platform.features.entropy import lob_entropy, earth_mover_distance
from hft_platform.features.fractal import hurst_exponent
from hft_platform.features.advanced_liquidity import RollingEffectiveSpread, RollingAmihud
...
def get_price(rows, level):
    if not rows or len(rows) <= level: return 0.0
    row = rows[level]
    if isinstance(row, dict): return row.get("price", 0.0)
    return row[0]
