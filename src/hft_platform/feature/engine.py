...
class _LobKernelState:
    ...
    # ISS (Impact Surprise Signal) state — v2
    ...
    # MLDM (Multi-Level Depth Momentum) state — v2
...
class FeatureEngine:
    ...
    def _compute_values(
        ...
        # Save previous BBO for MLDM guard before overwriting
        ...
        # v2 features: ISS + MLDM (only computed if feature set has >16 features)
        if len(self._feature_ids) <= 16:
            return v1_tuple

        iss_val = self._compute_iss(ks, ofi_l1_raw, mid_price_x2, bid_depth, ask_depth)
        mldm_val = self._compute_mldm(ks, event, best_bid, best_ask, _prev_bb_for_mldm, _prev_ba_for_mldm)

        return v1_tuple + (iss_val, mldm_val)
