...
from hft_platform.contracts.strategy import OrderIntent, RiskDecision, OrderCommand, StormGuardState
from hft_platform.risk.validators import PriceBandValidator, MaxNotionalValidator, StormGuardFSM
...
class RiskEngine:
    def __init__(self, config_path: str, intent_queue: asyncio.Queue, order_queue: asyncio.Queue):
        ...
        # Validators
        self.validators = [
            PriceBandValidator(self.config),
            MaxNotionalValidator(self.config)
        ]
        ...
        while self.running:
            try:
                ...
                if decision.approved:
                     ...
                     logger.warning("Order Rejected by Risk", 
                                    sid=intent.strategy_id, 
                                    reason=decision.reason_code)
                     self.metrics.risk_reject_total.labels(strategy=intent.strategy_id, reason=decision.reason_code).inc()
