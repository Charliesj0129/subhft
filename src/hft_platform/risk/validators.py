...
# StormGuardFSM: deprecated backward-compat shim for test migration.
# Production code should use StormGuard from hft_platform.risk.storm_guard.
class StormGuardFSM:
    """Deprecated. Use StormGuard instead. Kept only for test backward compatibility."""

    def __init__(self, config: Dict[str, Any]):
        from hft_platform.risk.storm_guard import StormGuard as _StormGuard

        self._guard = _StormGuard()
        self.config = config
    ...
    @state.setter
    def state(self, value: StormGuardState) -> None:
    ...
    def update_pnl(self, pnl: int) -> None:
        ...
        if pnl <= self.halt:
            ...
            target = StormGuardState.NORMAL
        # Escalation is always instant; de-escalation is immediate for compat
        self._guard.state = target
