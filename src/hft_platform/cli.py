...
def cmd_run(args: argparse.Namespace):
    ...
    if args.strategy:
        cli_overrides["strategy"] = {
            "id": args.strategy,
            "module": args.strategy_module or "hft_platform.strategies.demo_strategy",
            "class": args.strategy_class or "DemoStrategy",
            "params": {},
        }
...
def cmd_init(args: argparse.Namespace):
    ...
    strategy_tpl = textwrap.dedent(
        ...
        class Strategy(Strategy):
            default_params = {{"spread": 0.5, "qty": 1}}
            ...
            def on_book(self, ctx: StrategyContext, event: dict) -> List[OrderIntent]:
                if event.get("symbol") not in self.symbols:
                    return []
                feats = ctx.get_features(self.symbols[0])
                mid = feats.get("mid_price")
                spread = feats.get("spread", 0)
                if not mid or spread <= self.params["spread"]:
                    return []
                price = int(mid - spread / 2)
                ...
                logger.info("placing order", price=price, params=self.params)
                return [intent]
        """
    ).strip() + "\n"
    ...
    test_tpl = textwrap.dedent(
        ...
        class DummyCtx(StrategyContext):
            def __init__(self):
                super().__init__(
                    lob=None,
                    positions={{}},
                    ...
                    intent_factory=lambda **kwargs: OrderIntent(intent_id=1, timestamp_ns=0, **kwargs),
                    price_scaler=lambda symbol, price: int(price * 10000),
                    features={{"{symbol}": {{"mid_price": 10000, "spread": 1000}}}},
                )

        def test_strategy_emits_intent():
            strat = Strategy(strategy_id="{strategy_id}")
            ctx = DummyCtx()
            intents = strat.on_book(ctx, {{"symbol": "{symbol}", "type": "BidAsk"}})
...
def cmd_strat_test(args: argparse.Namespace):
    ...
    module_name = args.module or settings.get("strategy", {}).get("module", "hft_platform.strategies.demo_strategy")
    class_name = args.cls or settings.get("strategy", {}).get("class", "DemoStrategy")
    ...
    ctx = StrategyContext(
        ...
        price_scaler=lambda s, p: int(float(p) * 10000),
        features={symbol: {"mid_price": 10000, "spread": 1000}},
    )
...
def cmd_backtest(args: argparse.Namespace):
    ...
    if args.backtest_cmd == "run":
        try:
            from hft_platform.backtest.runner import HftBacktestRunner, HftBacktestConfig
        ...
        if args.strategy_module:
            ...
            adapter = StrategyHbtAdapter(
                ...
                strategy_class=args.strategy_class or "DemoStrategy",
...
def cmd_resolve_symbols(args: argparse.Namespace):
    ...
    import yaml
    try:
        from hft_platform.config.loader import load_settings, detect_live_credentials
    except ImportError:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    back_run.add_argument("--fee-taker", type=float, help="Taker fee (per value, negative for rebate)")
    back_run.add_argument("--no-partial-fill", action="store_true", help="Disable partial fill (use no_partial_fill_exchange)")
