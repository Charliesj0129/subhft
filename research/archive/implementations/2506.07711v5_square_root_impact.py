#!/usr/bin/env python3
"""
Square-root impact + decay sandbox driven by OFI.

- Generate OFI series from a simple LOB simulation
- Apply square-root impact with post-trade decay
- Output impact curve summaries
"""
...
def apply_square_root_impact(cfg: SimConfig, ofi: np.ndarray) -> Dict[str, np.ndarray]:
    n = ofi.size
    impact = np.zeros(n, dtype=np.float64)
    price = np.zeros(n, dtype=np.int64)
    price[0] = cfg.mid_price

    decay = np.exp(-cfg.decay_lambda * cfg.dt)
    for t in range(1, n):
        shock = cfg.impact_coef * np.sign(ofi[t]) * np.sqrt(abs(ofi[t])) * cfg.shock_scale
        impact[t] = impact[t - 1] * decay + shock
        price[t] = price[t - 1] + int(np.round(impact[t])) * cfg.tick_size
    return {"impact": impact, "price": price}


def summarize(impact: np.ndarray) -> Dict[str, float]:
    return {
        "impact_mean": float(impact.mean()),
        "impact_std": float(impact.std()),
        "impact_p95": float(np.percentile(impact, 95)),
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Square-root impact sandbox")
    ...
    parser.add_argument("--impact-coef", type=float, default=0.6)
    parser.add_argument("--decay-lambda", type=float, default=0.05)
    parser.add_argument("--shock-scale", type=float, default=1.0)
...
def main() -> None:
    ...
    impact_data = apply_square_root_impact(cfg, ofi)

    stats = summarize(impact_data["impact"])
    print("Square-root impact summary")
    print("impact_mean:", round(stats["impact_mean"], 6))
    print("impact_std:", round(stats["impact_std"], 6))
    print("impact_p95:", round(stats["impact_p95"], 6))
