...
def _load_pipeline_profile(profile_stem: str) -> ValidationProfile:
    ...
    if not path.exists():
        raise FileNotFoundError(f"validation profile YAML not found: {path} (expected under {_PROFILE_YAML_DIR}/)")
...
def _apply_validation_profile(
    ...
    if not strict_mode:
        notes.append(
            f"validation_profile={profile.name} requested in triage mode; output remains non-promotable by policy."
        )

    if applied:
        notes.append(f"Applied validation profile {profile.name} overrides: " + ", ".join(sorted(applied)))
    else:
        notes.append(
            f"validation_profile={profile.name} requested; no default-value fields were available to override."
        )
    return profile
