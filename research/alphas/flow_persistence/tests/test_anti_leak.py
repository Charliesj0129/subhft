"""Gate B anti-leak / lookahead-bias tests for FlowPersistenceAlpha."""
...
def test_update_is_deterministic() -> None:
    """Two separate runs with the same inputs produce identical output."""
    for _ in range(3):
        alpha = FlowPersistenceAlpha()
        sigs: list[float] = []
        for b, a in [(500.0, 100.0), (100.0, 500.0), (300.0, 300.0)]:
            sigs.append(alpha.update(b, a))
    # Last run's signals should match (deterministic)
    alpha2 = FlowPersistenceAlpha()
    for i, (b, a) in enumerate([(500.0, 100.0), (100.0, 500.0), (300.0, 300.0)]):
        assert alpha2.update(b, a) == sigs[i]
...
def test_extreme_bid_stays_finite() -> None:
...
def test_extreme_ask_stays_finite() -> None:
...
# ---------------------------------------------------------------------------
# 5. Reset eliminates state dependency
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Sustained direction (200 ticks)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. Bounds with random data (1000 ticks)
# ---------------------------------------------------------------------------


def test_bounds_1000_random_ticks() -> None:
    """Signal stays finite across 1000 random ticks."""
    alpha = FlowPersistenceAlpha()
...
# ---------------------------------------------------------------------------
# 9. No contamination between instances
# ---------------------------------------------------------------------------
