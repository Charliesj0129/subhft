...
Formula:
  ...
  qi_ema    = EMA_8(qi)          [bootstrap: first tick sets to raw]
  deviation = (qi - qi_ema)**2
  vol       = sqrt( EMA_16(deviation) )
  baseline  = EMA_64(vol)
  raw       = (vol - baseline) / max(baseline, eps)
...
_MANIFEST = AlphaManifest(
    ...
    formula=(
        "signal = clip((sqrt(EMA16((qi - EMA8(qi))^2)) - EMA64(vol))"
        " / max(EMA64(vol), eps) * sign(EMA8(qi)), -2, 2)"
    ),
    paper_refs=("064",),
...
class VolOfImbalanceAlpha:
    ...
    __slots__ = ("_qi_ema", "_dev_ema", "_vol_baseline", "_signal", "_initialized")
    ...
    def update(self, *args: float, **kwargs: float) -> float:
        ...
        # EMA of qi (directional) -- bootstrap on first tick
        ...
        sign_qi = (
            1.0 if self._qi_ema > 0.0 else (-1.0 if self._qi_ema < 0.0 else 0.0)
        )
