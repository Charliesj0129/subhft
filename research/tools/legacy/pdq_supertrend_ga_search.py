"""Genetic search over Supertrend + PDQ routing rules.

The search space is intentionally much larger than 100M combinations, but the
script does not brute-force all of them.  It uses a genetic algorithm and
optimizes fitness on IS only; OOS is reported for validation and is not used
for selection.
"""
...
ST_TOOL = ROOT / "research/tools/pdq_supertrend_backtest.py"
EVENTS_PATH = ROOT / "outputs/liquidity_score/pdq_supertrend_backtest/supertrend_event_features.csv"
OUT_DIR = ROOT / "outputs/liquidity_score/pdq_supertrend_ga_search"
...
FACTOR_VALUES = tuple(round(x, 2) for x in np.arange(0.5, 8.01, 0.1))
...
pdq = load_module(BASE_TOOL, "pdq_base_tool")
st_tool = load_module(ST_TOOL, "pdq_supertrend_tool")
...
class Evaluator:
    def __init__(self) -> None:
        ...
        self.events["e_TXF_dir"] = np.sign(self.events["e_TXF_t0"].to_numpy(dtype=float)).astype(np.int8)
        self._prepare_quantiles()
        self._prepare_arrays()
        self.bars_by_tf = self._build_bars()
        self.st_cache: dict[tuple[str, int, float], np.ndarray] = {}
    ...
    def _prepare_arrays(self) -> None:
        e = self.events
        self.split = e["split"].to_numpy(dtype=object)
        self.is_mask = self.split == "IS"
        self.oos_mask = self.split == "OOS"
        ...
        self.ret = {
            hold: e[f"ret_{hold}"].to_numpy(dtype=float)
        ...
        self.prev_bar = {
            tf: ((e["t0"].to_numpy(dtype=np.int64) // tf_s - 1) * tf_s)
            for tf, tf_s in TIMEFRAMES.items()
        }
    ...
    def _build_bars(self) -> dict[str, pd.DataFrame]:
        raw = pd.read_csv(pdq.DATA_PATH, usecols=["root", "day", "sec", "mid_pts"])
        secbar = raw.pivot_table(
            index=["day", "sec"],
            columns="root",
            values="mid_pts",
            aggfunc="first",
        ).reset_index()
        secbar = secbar.dropna(subset=["TXF", "MXF", "TMF"]).copy()
        secbar = secbar.sort_values(["day", "sec"], kind="mergesort").reset_index(drop=True)
        secbar["mid_agg"] = 0.5 * secbar["TXF"] + 0.3 * secbar["MXF"] + 0.2 * secbar["TMF"]
        out = {}
        for tf, tf_s in TIMEFRAMES.items():
            out[tf] = (
    ...
    def st_dir_for_gene(self, gene: Gene) -> np.ndarray:
        key = (gene.timeframe, gene.atr, gene.factor)
        cached = self.st_cache.get(key)
        if cached is not None:
            return cached
        bars = self.bars_by_tf[gene.timeframe]
        st = st_tool.compute_supertrend_bars(bars, gene.atr, gene.factor)
        mapping = dict(zip(st["bar_start"].to_numpy(dtype=np.int64), st["st_dir"].to_numpy(dtype=np.int8), strict=False))
        dirs = np.array([mapping.get(int(bar), 0) for bar in self.prev_bar[gene.timeframe]], dtype=np.int8)
        self.st_cache[key] = dirs
        return dirs

    def direction(self, gene: Gene) -> np.ndarray:
        st = self.st_dir_for_gene(gene)
        ...
        if gene.direction_mode == "ST_C_TSI_ETXF":
            return np.where(
                (st != 0) & (st == self.c_dir) & (st == self.tsi_dir) & (st == self.etxf_dir),
                st,
                0,
            ).astype(np.int8)
    ...
    def mask(self, gene: Gene, direction: np.ndarray) -> np.ndarray:
        mask = direction != 0
        hours = np.array([(gene.hour_mask >> int(h)) & 1 for h in self.hour], dtype=bool)
        mask &= hours
        if gene.abs_c_q != -1:
...
def fitness_from_is(summary: dict[str, float | int]) -> float:
    n = int(summary["n"])
    days = int(summary["active_days"])
    if n < 80 or days < 15 or not math.isfinite(float(summary["net_cost4"])):
        return -1_000.0 + n * 0.01 + days * 0.1
    net = float(summary["net_cost4"])
    robust = float(summary["drop_top3_net_cost4"])
    hit = float(summary["hit_rate"])
    concentration = float(summary["top5_day_abs_share"])
    sample_bonus = min(n, 600) / 600.0
...
def random_gene(rng: random.Random) -> Gene:
    return Gene(
        timeframe=rng.choice(tuple(TIMEFRAMES.keys())),
        atr=rng.choice(ATR_VALUES),
...
def mutate(g: Gene, rng: random.Random, rate: float) -> Gene:
    vals = asdict(g)
    if rng.random() < rate:
        vals["timeframe"] = rng.choice(tuple(TIMEFRAMES.keys()))
    if rng.random() < rate:
        vals["atr"] = max(min(vals["atr"] + rng.choice([-5, -3, -2, -1, 1, 2, 3, 5]), max(ATR_VALUES)), min(ATR_VALUES))
    if rng.random() < rate:
        vals["factor"] = min(max(round(vals["factor"] + rng.choice([-0.5, -0.3, -0.2, -0.1, 0.1, 0.2, 0.3, 0.5]), 2), min(FACTOR_VALUES)), max(FACTOR_VALUES))
        vals["factor"] = min(FACTOR_VALUES, key=lambda x: abs(x - vals["factor"]))
    if rng.random() < rate:
        vals["direction_mode"] = rng.choice(DIRECTION_MODES)
    if rng.random() < rate:
        vals["hold_s"] = rng.choice(HOLD_VALUES)
    if rng.random() < rate:
        if rng.random() < 0.6:
            ...
            vals["hour_mask"] = random_hour_mask(rng)
    for key in ["abs_c_q", "rvexp_q", "spread_q", "d5_q"]:
        if rng.random() < rate:
            vals[key] = rng.choice(THRESH_VALUES)
    for key in ["no_toxic", "require_clean_proxy", "require_reverse_proxy"]:
        if rng.random() < rate:
            vals[key] = not vals[key]
    if rng.random() < rate:
        vals["cshape_mode"] = rng.choice(CSHAPE_MODES)
    if rng.random() < rate:
        vals["liquidity_mode"] = rng.choice(LIQ_MODES)
    if rng.random() < rate:
        vals["vwap_mode"] = rng.choice(VWAP_MODES)
    if rng.random() < rate:
        vals["or_mode"] = rng.choice(OR_MODES)
    return Gene(**vals)


def gene_key(g: Gene) -> tuple[Any, ...]:
    return tuple(asdict(g).values())


def theoretical_space_size() -> int:
    # Hour mask excludes zero, but use 2^24 - 1 for the full discrete space.
    return (
        len(TIMEFRAMES)
        * len(ATR_VALUES)
        * len(FACTOR_VALUES)
...
def run_ga(args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    def eval_gene(g: Gene) -> dict[str, Any]:
        key = gene_key(g)
        row = eval_cache.get(key)
        if row is None:
            row = evaluator.evaluate(g)
            row["hours"] = hours_from_mask(g.hour_mask)
            eval_cache[key] = row
            all_rows.append(row)
        return row
    ...
    for generation in range(args.generations):
        ...
        scored_sorted = sorted(scored, key=lambda r: float(r["fitness"]), reverse=True)
        best = dict(scored_sorted[0])
        best["generation"] = generation
        best["unique_evaluations"] = len(eval_cache)
        best["st_cache_size"] = len(evaluator.st_cache)
        best_rows.append(best)

        elites_n = max(2, int(args.population * args.elite_frac))
        elites = [
            Gene(**{field: row[field] for field in Gene.__dataclass_fields__})
            for row in scored_sorted[:elites_n]
        ]
        next_pop = elites[:]
        tournament_pool = [
            Gene(**{field: row[field] for field in Gene.__dataclass_fields__})
            for row in scored_sorted[: max(elites_n * 4, 8)]
        ]
        while len(next_pop) < args.population:
            a = max(rng.sample(tournament_pool, k=min(4, len(tournament_pool))), key=lambda g: eval_gene(g)["fitness"])
            b = max(rng.sample(tournament_pool, k=min(4, len(tournament_pool))), key=lambda g: eval_gene(g)["fitness"])
            child = crossover(a, b, rng)
            child = mutate(child, rng, args.mutation_rate)
    ...
    meta = {
        ...
        "unique_evaluations": int(len(eval_cache)),
        "supertrend_cache_size": int(len(evaluator.st_cache)),
        "theoretical_search_space": int(theoretical_space_size()),
        "theoretical_search_space_gt_100m": theoretical_space_size() > 100_000_000,
        "fitness": "IS only: min(net_cost4, drop_top3_net_cost4) plus sample/day/hit bonuses and concentration penalty",
        "oos_usage": "reported only; not used for GA selection",
        "cost_points": COST_POINTS,
        "source_events": str(EVENTS_PATH.relative_to(ROOT)),
        "price_unit": "TAIFEX points",
    }
    return all_df, best_df, meta
...
def write_html(tables: dict[str, pd.DataFrame], meta: dict[str, Any]) -> None:
    ...
    body = [
        "<!doctype html><html><head><meta charset='utf-8'><title>PDQ Supertrend GA Search</title>",
        f"<style>{css}</style></head><body>",
        "<h1>PDQ Supertrend GA Search</h1>",
        "<p class='note'>GA searches a theoretical parameter space greater than 100M combinations. Selection fitness uses IS only; OOS is reported for validation. This is a path-level taker proxy, not bid/ask/latency fill simulation.</p>",
...
def main() -> None:
    ...
    top_is = all_df.sort_values("fitness", ascending=False).head(200)
    top_oos = all_df[
        (all_df["oos_n"] >= 80)
        & (all_df["oos_active_days"] >= 15)
        & np.isfinite(all_df["oos_net_cost4"])
    ].sort_values("oos_drop_top3_net_cost4", ascending=False).head(200)
    ...
    write_html(
        {
            "Top by IS Fitness": top_is[cols].head(50),
            "Top by OOS Robustness": top_oos[cols].head(50),
            "Best by Generation": best_df[["generation", "unique_evaluations", *cols]].tail(30),
        },
        meta,
    )
