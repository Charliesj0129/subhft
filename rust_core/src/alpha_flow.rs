...
impl MatchedFilterTradeFlow {
    ...
    pub fn update(&mut self, trade_vol: f64, trade_side: f64) -> f64 {
        ...
        if self.trade_vol_history.len() > self.fast_window {
            let old_vol = self.trade_vol_history[self.trade_vol_history.len() - 1 - self.fast_window];
            let old_side = self.trade_side_history[self.trade_side_history.len() - 1 - self.fast_window];
