...
/// Fused feature pipeline: computes all 16 feature values + changed_mask +
/// warmup_ready_mask in a single Rust call.  Replaces three separate Python
/// methods (`_compute_values`, `_compute_changed_mask`,
/// `_compute_warmup_ready_mask`) with one cross-boundary call.
#[pyclass]
pub struct RustFeaturePipelineV1 {
    kernel: LobFeatureKernelV1Inner,
    warmup_thresholds: Vec<i64>,
    prev_values: Option<Vec<i64>>,
}
...
#[pymethods]
impl RustFeaturePipelineV1 {
    /// Create a new pipeline with per-feature warmup thresholds.
    #[new]
    pub fn new(warmup_thresholds: Vec<i64>) -> Self {
        Self {
            kernel: LobFeatureKernelV1Inner::new(),
            warmup_thresholds,
            prev_values: None,
        }
    }

    /// Fused compute: returns (values, changed_mask, warmup_ready_mask).
    #[allow(clippy::too_many_arguments)]
    pub fn process(
        &mut self,
        best_bid: i64,
        ...
        let values = self.kernel.compute(
            best_bid,
            best_ask,
            mid_price_x2,
            spread_scaled,
        ...
        // Compute changed_mask
        let changed_mask = match &self.prev_values {
            None => (1_i64 << values.len()) - 1,
            Some(prev) => {
                if prev.len() != values.len() {
                    (1_i64 << values.len()) - 1
                } else {
                    let mut mask: i64 = 0;
                    for (idx, (a, b)) in prev.iter().zip(values.iter()).enumerate() {
                        if a != b {
                            mask |= 1 << idx;
                        }
                    }
        ...
        // Compute warmup_ready_mask
        let mut warmup_mask: i64 = 0;
        for (idx, threshold) in self.warmup_thresholds.iter().enumerate() {
        ...
        (values, changed_mask, warmup_mask)
    }
