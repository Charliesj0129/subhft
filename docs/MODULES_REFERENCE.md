...
This document is a compressed directory map of `src/hft_platform/` (37 packages, ~210 files).

<!-- AUTO-GENERATED from codebase scan 2026-04-01 -->
...
| **feed_adapter/shioaji/** | `ShioajiClientFacade`, `ShioajiClient` (44KB), `TickDispatcher`, `ReconnectOrchestrator` + 16 more | **20 files.** Full Shioaji sub-package: session/quote/order/account/contracts runtimes, tick dispatch (async worker thread), reconnect orchestration, multi-client routing, WebSocket connection pool. |
...
| **order/** | `OrderAdapter` (53KB), `CircuitBreaker`, `DeadLetterQueue`, `HaltCanceller`, `ShadowOrderSink` | **6 files.** Order dispatch + lifecycle: rate limiting (180/250 per 10s), 5ms coalescing, per-strategy circuit breaker, shadow mode, DLQ (TTL 30s). |
| **execution/** | `ExecutionRouter`, `PositionStore`, `ExecutionNormalizer`, `ExecutionOptimizer`, `ImbalanceTimer`, `RegimeClassifier` | **15 files.** Fill normalization, integer-only position tracking (`RustPositionTracker`), startup/EOD reconciliation, checkpoint, limit-vs-market decision (Albers 2025), LOB imbalance timing (IC=+0.116), regime classification, slippage tracking, MTM, fill DLQ. |
| **gateway/** | `GatewayService`, `ExposureStore`, `IdempotencyStore`, `GatewayPolicy`, `LeaderLease` | **6 files.** CE-M2 optional gateway: 7-step synchronous dispatch (dedup, policy, exposure, risk, command, dispatch, commit). Bounded 10K entries with eviction. `HFT_GATEWAY_ENABLED=0` (default off). |
...
| Subpackage | Key Classes / Files | Responsibility |
|---|---|---|
| **services/** | `HFTSystem`, `SystemBootstrapper`, `MarketDataService` (5 mixins), `ServiceRegistry`, `HeartbeatService` | **11 files.** App lifecycle: builds 5 bounded queues + 18 services. Supervision loop (lag/depth/liveness). SIGTERM/SIGINT handler. |
...
| **core/** | `timebase`, `pricing.PriceCodec`, `order_ids`, `instrument_registry`, `market_calendar`, `rate_limiter`, `secret_validator`, `session_hooks` | **8 files.** `now_ns()` (mandatory). PriceCodec (x10000 scaled int). Trading calendar. Rate limiter. |
...
| **migrations/** | `clickhouse/*.sql` (15 files, 619 lines) | ClickHouse DDL management. Tables: market_data (6mo TTL), orders/fills (1yr), audit (2yr). Auto-applied on boot. |
| **observability/** | `MetricsRegistry` (100+ metrics), `HealthServer`, `LatencySpan` | **5 files.** Prometheus metrics. HTTP health endpoints (`/healthz`, `/readyz`, `/status`). Pipeline latency tracking. |
| **notifications/** | `NotificationDispatcher`, `TelegramSender`, `WebhookSender`, `AlertManagerBridge` | **5 files.** Critical (HALT, daily loss) + normal event routing. Telegram/Webhook/AlertManager. |
...
| Subpackage | Key Classes / Files | Responsibility |
|---|---|---|
| **ops/** | `SessionGovernor`, `TrackGate`, `AutonomyMonitor`, `PositionFlattener`, `MarginMonitor`, `BackupManager` | **14 files.** Session phase FSM (INIT to CLOSED). Autonomy degradation (NORMAL to HALT). Emergency flattening. Margin monitoring. Config snapshot. Daily PnL report. Manual rearm. |
...
| **alpha/** | `validation.py` (57KB), `promotion.py` (39KB), `canary.py`, `experiments.py`, `pool.py`, `paper_trade_runner.py`, `latency_audit.py`, `screener.py`, `audit.py` | **18+ files.** 6-gate alpha governance: Gate A (data), B (statistical), C (backtest), D (quant threshold), E (paper-trade), F (Rust readiness). |
| **backtest/** | `BacktestRunner`, `BacktestAdapter`, `EquityTracker` | **10 files.** HftBacktest integration: JSONL to NPZ convert, feed/elapse loops, equity tracking, scorecard reporting. |
...
| **reports/** | `ReportingPipeline`, `ReportCollector`, `ReportComposer`, `ReportDistributor`, `FactExtractor`, `ReportReasoner` | **9 files.** Daily market report pipeline: collect, extract facts, reason, compose, distribute. Rules: `informed_flow`, `scenario_rules`, `support_resistance`. |
| **tca/** | `SlippageAnalyzer`, `FeeCalculator`, `SlippageRecord` | **5 files.** Transaction Cost Analysis: decision-vs-fill slippage, maker/taker fees, reporting. |
