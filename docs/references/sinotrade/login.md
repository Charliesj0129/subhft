...
### Fetch Contracts Callback[¶](#fetch-contracts-callback "Permanent link")
...
### Subscribe Trade[¶](#subscribe-trade "Permanent link")
...
### List Accounts[¶](#list-accounts "Permanent link")
...
* If `signed` does not appear in the account list, like ACCOUNT\_ID\_2 and ACCOUNT\_ID\_3, it means that the account has not signed or completed the test report in the simulation mode. Please refer to [Terms of service](../prepare/terms/).

### Default Account[¶](#default-account "Permanent link")
...
In order to provide high quality services, starting from 2021/08/06,
we've limit the [number of connections used](../limit/). It's a good practice
to logout or to terminate the program when it is not in use.
