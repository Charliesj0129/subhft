...
feature-parity: build-rust ## Run cross-path feature parity gate (Python/Rust/hftbacktest)
	uv run python -m hft_platform.cli feature parity --require-rust
...
# VERSIONS is overridable because the hardcoded list is why this was never
# re-run: `make shioaji-watch` prints the exact invocation for whatever is new.
SHIOAJI_VERSIONS ?= 1.2.9 1.3.3 1.5.3 1.5.5 1.5.6 1.5.7 1.7.0 1.7.1 1.7.2 1.7.3 1.7.4

shioaji-watch: ## List PyPI shioaji releases newer than the pin (exit 1 if any awaits a decision)
	$(PY) -m scripts.shioaji_api_diff watch --strict

shioaji-decision-evidence: ## Rebuild the pin-to-candidate diff each upgrade decision cites
	$(PY) -m scripts.shioaji_api_diff decision-evidence

shioaji-surface: ## Capture shioaji API surfaces across versions in throwaway /tmp venvs
	$(PY) -m scripts.shioaji_api_diff orchestrate --versions $(or $(VERSIONS),$(SHIOAJI_VERSIONS)) --jobs 5
...
shioaji-diff: ## Build machine diffs + the human runbook (docs/runbooks/shioaji-version-diff.md)
	$(PY) -m scripts.shioaji_api_diff report --versions $(or $(VERSIONS),$(SHIOAJI_VERSIONS))
...
research-data-quality: ## Audit raw ClickHouse market-data quality (advisory; writes JSON+MD report)
	@if [ -z "$(DATE_FROM)" ] || [ -z "$(DATE_TO)" ]; then \
		echo "Usage: make research-data-quality DATE_FROM=2026-01-26 DATE_TO=2026-08-04 [ARGS='--fail-on error']"; \
		exit 2; \
	fi
	$(PY) -m research data-pipeline quality --date-from "$(DATE_FROM)" --date-to "$(DATE_TO)" $(ARGS)

research-archive-sync: ## Report (dry-run) the local market-data archive against the upstream host
	$(PY) scripts/sync_market_data_archive.py --emit-inventory research/reports/data_quality/theshow_inventory.json $(ARGS)
