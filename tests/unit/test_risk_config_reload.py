...
class TestRiskConfigReload:
    def _make_risk_engine(self, config=None):
        config = config or {"global_defaults": {"max_price_cap": 5000.0, "tick_size": 0.01, "price_band_ticks": 20}, "strategies": {}}
        tmp = tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False)
        yaml.dump(config, tmp)
        tmp.close()
        with patch("hft_platform.risk.engine.MetricsRegistry") as m, \
    ...
    def test_storm_guard_reload_thresholds(self):
        from hft_platform.risk.storm_guard import StormGuard, RiskThresholds
