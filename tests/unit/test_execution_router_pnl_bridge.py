...
def _make_router(*, risk_engine=None):
    ...
    return ExecutionRouter(bus=bus, raw_queue=raw_queue, order_id_map={},
        position_store=position_store, terminal_handler=terminal_handler, risk_engine=risk_engine)
...
async def test_router_calls_notify_fill_pnl_on_fill():
    ...
    fill_norm = SimpleNamespace(account_id="a", strategy_id="s", symbol="X", order_id="o", fill_id="f", price=100, qty=1, side=1, fee=0, tax=0)
