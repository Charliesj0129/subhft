...
from research.backtest.maker_engine import SimpleMakerStrategy, TickData, PostQuote, Hold


def test_posts_quotes_when_spread_above_threshold():
    s = SimpleMakerStrategy(spread_threshold_pts=5, max_pos=1)
    tick = TickData(
        exch_ts=1, bid_price=100_000_000, ask_price=105_000_000,
        bid_qty=10, ask_qty=10, trade_price=0, trade_volume=0,
        is_trade=False, scale=1_000_000,
    )
...
def test_holds_when_spread_below_threshold():
    s = SimpleMakerStrategy(spread_threshold_pts=5, max_pos=1)
    tick = TickData(
        exch_ts=1, bid_price=100_000_000, ask_price=103_000_000,
        bid_qty=10, ask_qty=10, trade_price=0, trade_volume=0,
        is_trade=False, scale=1_000_000,
    )
    actions = s.on_tick(tick)
