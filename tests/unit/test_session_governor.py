...
class TestFlattenTaskDoneCallback:
    ...
    async def test_flatten_task_failure_logs_critical(self, tmp_path: Path, capsys: pytest.CaptureFixture) -> None:
        ...
        assert "session_flatten_task_failed" in captured.err, (
            "Expected 'session_flatten_task_failed' in log output but got: " + captured.err
        )
    ...
    async def test_flatten_task_success_does_not_log_error(self, tmp_path: Path, capsys: pytest.CaptureFixture) -> None:
        ...
        captured = capsys.readouterr()
        assert "session_flatten_task_failed" not in captured.err, "Unexpected flatten failure log: " + captured.err
    ...
    async def test_flatten_task_cancelled_logs_warning(self, tmp_path: Path, capsys: pytest.CaptureFixture) -> None:
        ...
        captured = capsys.readouterr()
        assert "session_flatten_task_cancelled" in captured.err, (
            "Expected 'session_flatten_task_cancelled' in log output but got: " + captured.err
        )
