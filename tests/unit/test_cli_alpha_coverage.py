...
class TestCmdAlphaList:
    ...
    def test_lists_alphas(self, capsys):
        ...
        fake_mod.AlphaDiscoveryRegistry.assert_called_once_with()
...
class TestCmdAlphaValidateBatch:
    ...
    def test_batch_all_pass(self, capsys):
        ...
        with patch.dict(
            "sys.modules",
            {"hft_platform.alpha.discovery": reg_mod, "hft_platform.alpha.validation": val_mod},
        ):
            cmd_alpha_validate_batch(self._base_args())
        out = capsys.readouterr().out
        assert "2 alphas" in out
        assert "passed" in out
        reg_mod.AlphaDiscoveryRegistry.assert_called_once_with()
