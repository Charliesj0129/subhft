...
class TestLoadLatencyProfiles:
    def test_loads_real_profiles_yaml(self) -> None:
        """Sanity: the real project profiles file loads without error."""
        profiles = _load_latency_profiles(".")
        assert isinstance(profiles, dict)
        assert _PROFILE_ID in profiles
    ...
    def test_returns_empty_dict_on_invalid_yaml(self, tmp_path: Path) -> None:
        config_dir = tmp_path / "config" / "research"
        config_dir.mkdir(parents=True)
        (config_dir / "latency_profiles.yaml").write_text("{{not valid yaml:")
        profiles = _load_latency_profiles(str(tmp_path))
        assert profiles == {}
