...
def _reset_metrics():
    ...
    if MetricsRegistry._instance is None:
        # Unregister all collectors to allow fresh MetricsRegistry creation
...
def _make_runner(
    ...
    """Create an EODReconciliationRunner with a mocked ReconciliationService."""
    recon = MagicMock()
...
async def test_triggers_sync_at_correct_hour():
    """sync_portfolio is called when UTC hour matches close_hour_utc."""
...
async def test_skips_wrong_hour():
    """sync_portfolio is NOT called when UTC hour does not match."""
    runner, sync_mock = _make_runner(close_hour=5)
...
async def test_once_per_day_guard():
    """sync_portfolio fires only once per calendar day."""
    runner, sync_mock = _make_runner(close_hour=5)
...
async def test_triggers_again_on_new_day():
    """sync_portfolio fires again on a different calendar day."""
    runner, sync_mock = _make_runner(close_hour=5)
...
async def test_metrics_updated_on_success():
    """eod_recon_status=1 and eod_recon_last_ts updated after success."""
...
async def test_metrics_updated_on_failure():
    """eod_recon_status=2 after sync_portfolio raises."""
...
async def test_configurable_hour():
    """Close hour can be set to a non-default value."""
    runner, sync_mock = _make_runner(close_hour=14)

    # Should skip at hour 5
    with patch(
        "hft_platform.execution.eod_recon._current_utc_hour_and_date",
        return_value=(5, "2026-03-19"),
    ):
...
async def test_run_loop_stops():
    """run() exits when running is set to False."""
    runner, _ = _make_runner(close_hour=5)
    ...
    with patch(
        "hft_platform.execution.eod_recon._POLL_INTERVAL_S",
        0.02,
    ):
...
async def test_env_var_default(monkeypatch: pytest.MonkeyPatch):
    """HFT_EOD_CLOSE_HOUR_UTC env var is respected."""
