...
# ---------------------------------------------------------------------------
# _build_feature_engine — disabled path
# ---------------------------------------------------------------------------


def test_build_feature_engine_disabled(monkeypatch):
    """_build_feature_engine returns all-None tuple when engine is disabled."""
    monkeypatch.setenv("HFT_FEATURE_ENGINE_ENABLED", "0")

    from hft_platform.services.bootstrap import SystemBootstrapper

    bs = SystemBootstrapper(settings={})
    result = bs._build_feature_engine()

    # All five elements should be None
    assert result == (None, None, None, None, None)


def test_build_feature_engine_enabled_no_registry(monkeypatch):
    """_build_feature_engine enabled but registry fails → falls back gracefully."""
    ...
    with patch("hft_platform.services.bootstrap.load_feature_profile_registry", side_effect=Exception("no registry")):
        with patch("hft_platform.services.bootstrap.load_feature_rollout_controller", side_effect=Exception("no ctrl")):
            with patch("hft_platform.services.bootstrap.FeatureEngine") as MockFE:
                mock_fe = MagicMock()
                mock_fe.feature_set_id.return_value = "lob_shared_v3"
                MockFE.return_value = mock_fe
                result = bs._build_feature_engine()

    # engine should be present (FeatureEngine constructed), registry/profile None
    assert result[0] is mock_fe
    assert result[1] is None  # profile_registry is None after failure


def test_build_feature_engine_rollout_assignment_disabled(monkeypatch):
    """_build_feature_engine sets profile=None when assignment.state=='disabled'."""
    monkeypatch.setenv("HFT_FEATURE_ENGINE_ENABLED", "1")
    ...
    mock_ctrl.get.return_value = mock_assignment
    mock_ctrl.resolve_profile_id.return_value = None

    with patch("hft_platform.services.bootstrap.load_feature_profile_registry", return_value=mock_registry):
        with patch("hft_platform.services.bootstrap.load_feature_rollout_controller", return_value=mock_ctrl):
            with patch("hft_platform.services.bootstrap.FeatureEngine") as MockFE:
                mock_fe = MagicMock()
                mock_fe.feature_set_id.return_value = "lob_shared_v3"
                MockFE.return_value = mock_fe
    ...
    # Feature engine present, profile is None (disabled)
    assert result[0] is mock_fe
    assert result[2] is None  # feature_profile is None
