...
def test_order_intent_default_price_type_is_lmt():
    intent = OrderIntent(intent_id=1, strategy_id="test", symbol="TXO22500D6",
        intent_type=IntentType.NEW, side=Side.BUY, price=225000000, qty=1)
...
def test_order_intent_backward_compat_no_price_type_arg():
    intent = OrderIntent(intent_id=1, strategy_id="test", symbol="TXFD6",
        intent_type=IntentType.NEW, side=Side.BUY, price=200000000, qty=1)
    assert intent.price_type == "LMT"
...
def test_risk_feedback_dataclass():
    ...
    fb = RiskFeedback(intent_id=42, strategy_id="eye", symbol="TXO22500D6",
        reason_code="GREEKS_DELTA_LIMIT", timestamp_ns=1000000000)
