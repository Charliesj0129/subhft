...
def test_recover_dual_source_match(tmp_path):
    ...
    verifier = StartupPositionVerifier(
        client=_make_client(broker_positions), position_store=store, checkpoint_path=ckpt_path,
    )
...
def test_recover_minor_discrepancy_auto_corrects(tmp_path):
    ...
    store = _make_store()
    verifier = StartupPositionVerifier(
        client=_make_client(broker_positions), position_store=store,
        checkpoint_path=ckpt_path, qty_threshold=10,
    )
    result = asyncio.run(verifier.recover(trading_date="20260325", account_id="test"))
    assert result.source == "dual"
...
def test_recover_broker_unavailable_checkpoint_only(tmp_path):
    ...
    store = _make_store()
    verifier = StartupPositionVerifier(
        client=client, position_store=store, checkpoint_path=ckpt_path,
    )
    result = asyncio.run(verifier.recover(trading_date="20260325", account_id="test"))
...
def test_recover_both_unavailable_halts(tmp_path):
    ...
    store = _make_store()
    verifier = StartupPositionVerifier(
        client=client, position_store=store, checkpoint_path=str(tmp_path / "nonexistent.json"),
    )
    result = asyncio.run(verifier.recover(trading_date="20260325", account_id="test"))
    assert result.halted is True
