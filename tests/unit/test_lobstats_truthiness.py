...
class TestLOBStatsEventZeroPrice:
    ...
    def test_zero_best_bid_computes_mid_price(self):
        ...
        event = LOBStatsEvent(
            symbol="TEST", ts=1, imbalance=0.0,
            best_bid=0, best_ask=1000000, bid_depth=10, ask_depth=20,
        )
    ...
    def test_zero_best_ask_computes_mid_price(self):
        ...
        event = LOBStatsEvent(
            symbol="TEST", ts=1, imbalance=0.0,
            best_bid=1000000, best_ask=0, bid_depth=10, ask_depth=20,
        )
        assert event.mid_price_x2 == 1000000
        assert event.spread_scaled == -1000000
    ...
    def test_both_zero_computes_zero(self):
        ...
        event = LOBStatsEvent(
            symbol="TEST", ts=1, imbalance=0.0,
            best_bid=0, best_ask=0, bid_depth=0, ask_depth=0,
        )
    ...
    def test_normal_values_unchanged(self):
        ...
        event = LOBStatsEvent(
            symbol="2330", ts=1, imbalance=0.5,
            best_bid=5000000, best_ask=5010000, bid_depth=100, ask_depth=200,
        )
