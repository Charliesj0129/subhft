...
from hft_platform.contracts.strategy import OrderCommand, OrderIntent, IntentType, Side, TIF
...
class TestOrderLifecycle(unittest.TestCase):
    ...
    def test_lifecycle(self, mock_load):
        ...
        # 1. Create Order
        intent = OrderIntent(
            intent_id=1, strategy_id="strat1", symbol="2330", 
            side=Side.BUY, price=100, qty=1, intent_type=IntentType.NEW, 
            tif=TIF.LIMIT
        )
        ...
        self.assertEqual(adapter.order_id_map["123"], "strat1")
    ...
    def test_cancel_flow(self, mock_load):
        ...
        # Place Parent
        intent = OrderIntent(
            intent_id=2, strategy_id="strat1", symbol="2330", 
            side=Side.BUY, price=100, qty=1, intent_type=IntentType.NEW, 
            tif=TIF.LIMIT
        )
        ...
        cancel_intent = OrderIntent(
            intent_id=3, strategy_id="strat1", symbol="2330",
            side=Side.BUY, price=0, qty=0, intent_type=IntentType.CANCEL,
            target_order_id=2
        )
